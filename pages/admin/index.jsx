...
const Index = ({ orders, products }) => {
  const [bookList, setBookList] = useState(products);
  const [orderList, setOrderList] = useState(orders);
  const status = ['packaging', 'on the way', 'delivered'];
          ...
          {orderList.map((order) => (
            <tbody key={order._id}>
              <tr className={styles.trTitle}>
                ...
                <td>XAF{order.total}</td>
                <td>
                  {orders.method === 0 ? <span>cash</span> : <span>paid</span>}
                </td>
